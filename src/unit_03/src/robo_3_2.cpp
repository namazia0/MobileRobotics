...
void A_star::poseAMCLCallback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& msgAMCL)
{
...
void A_star::calculateCommand() {
    ...
    double dx= zx-poseAMCLx;
    double dy= zy-poseAMCLy;
    
    
    double alpha = atan2 (dy,dx);
    
    angularz= 0.1*(alpha-poseAMCLa);

  
    double diffalpha = alpha - poseAMCLa;
    /*
    //line up first, then move
    if (direction > 0.5){
     angularz=0.05;   
     linearx=0.0;
    }
...
//     }
    ...
    ROS_INFO(" alpha = %f, angularz= %f, dx=%f, dy=%f, diffAlpha=%f \n", alpha, angularz, dx,dy, diffalpha  ) ;
...
void A_star::mainLoop() {
	...
	while (m_nodeHandle.ok())
	{
	   ...
	   /* Plan  */
		...
		ROS_INFO(" robo_3_2 commands:forward speed=%f[m/sec] and turn speed =%f[rad/sec] and amcl posx=%f, posy=%f, yaw=%f", 
