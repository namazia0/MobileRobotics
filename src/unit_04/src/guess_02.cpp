...
// TASK:
// Mean value of all coordinates with blue pixels. Searches for blue pixels.
// The mobile robot moves in the direction where the blue object is located in the camera.
...
// Aufruf ist : do_some_magic( frame , 225, 50, 75, 100);
...
void color_control::do_some_magic( IplImage* img, int red, int green, int blue, int tolerance) {
    ...
    // Zur Kontrolle einen Kreis einzeichnen und das Kontroll-Bild ausgeben
    CvPoint center;
    center.x = S_x;
    center.y = S_y;
...
void color_control::mainLoop() {
	...
	while (m_nodeHandle.ok())
	{
		    ...
		    do_some_magic( frame , 255, 192, 203, 100);
